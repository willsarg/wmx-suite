...
Add a speed selector that trades fit granularity for fewer model loads, primarily by
running **fewer ramp rungs**. Coarse fitting is an accepted tradeoff.
...
| Preset     | Ramp (rungs)                              | Repeats | Max loads | vs standard |
|------------|-------------------------------------------|---------|-----------|-------------|
| `quick`    | `[2048, 16384, 65536, 131072]` (4)        | 2       | ~8        | ~3× faster  |
| `standard` | current `DEFAULT_RAMP` (8)                 | 3       | ~24       | baseline    |
| `full`     | `[2048, 4096, 8192, 16384, 24576, 32768, 49152, 65536, 98304, 131072]` (10) | 3 | ~30 | finer, slower |
...
Honest tradeoff: with `quick`, the first prediction leans on a 2-point fit across a large
gap (2048 → 16384 → 65536), so the *accuracy* of the extrapolated safe ceiling is
rougher. `repeats=2` keeps a robustness floor so a single transient memory spike cannot
solely tilt the safety fit.
